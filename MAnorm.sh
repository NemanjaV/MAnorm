...
then
  ...
  if [ -e $(basename $PEAKS1 .bed).xls -a -e $(basename $PEAKS2 .bed).xls ]
  then
...
OUTPUT=$(echo $3 | grep -oP '.*(?=_[12][0-9][012][0-9][0123][0-9])')_vs_$(echo $4 | grep -oP '.*(?=_[12][0-9][012][0-9][0123][0-9])') # CHANGE, only for our server
...
if [[ $READS1 == "*bam" ]]; then
	echo "The provided file $READS1 a BAM file. It will be converted to BED"
	# Simply replace 4 (strand) with 6
	bedtools bamtobed -i $3 | awk -v var=$BP1 'BEGIN {OFS="\t"}
...
echo "MAnorm.sh execution finished. Files saved as $OUTPUT\n"
