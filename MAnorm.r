# NOTE: This R script has been modified to load R.utils instead of R.basic, which is no longer available from its original source.
...
pval <- function(x, y){
	if (x+y<20) { # x + y is small
		p1<- choose(x+y,x) * 2^-(x+y+1); # Originally nChooseK, but choose does the same thing
